...
def solve_1d_linearconv(u, un, nt, nx, dt, dx, c):
    for n in range(nt):  
        ...
        # Next batch inter-change
        # if RANK < SIZE-1:
        #     COMM.send(u[-1], dest=RANK+1, tag=2)
        # if RANK != 0:
        #     u[0] = COMM.recv(source=RANK-1, tag=2)
        
        # Boundaries conditions: u[0] = u[nx-1]
        # if RANK == SIZE-1:
        #     COMM.send(u[-1], dest=0, tag=3)
        # if RANK == 0:
        #     u[0] = COMM.recv(source=SIZE-1, tag=3)

        # Next batch inter-change AND Boundaries conditions(u[0] = u[nx-1])
