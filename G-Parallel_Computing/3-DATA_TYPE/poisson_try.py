...
def create_derived_type(sx, ex, sy, ey):
    ...
    type_line = MPI.DOUBLE.Create_contiguous(ex - sx + 3)
    type_line.Commit()

    '''Creation of the type_column derived datatype to exchange points
     with western to eastern neighbours '''
    stride = ex - sx + 3
    type_column = MPI.DOUBLE.Create_vector(ey - sy + 3, 1, stride)
    type_column.Commit()
...
def communications(u, sx, ex, sy, ey, type_column, type_ligne):
    ...
    cart2d.Sendrecv([u[ex-sx+3:], 1, type_ligne], dest=neighbour[N], recvbuf=[u[(ex-sx+3)*(ey-sy+2):], 1, type_ligne], source=neighbour[S])

    ''' Send to neighbour S and receive from neighbour N '''
    cart2d.Sendrecv([u[(ex-sx+3)*(ey-sy+1):], 1, type_ligne], dest=neighbour[S], recvbuf=[u, 1, type_ligne], source=neighbour[N])

    ''' Send to neighbour W and receive from neighbour E '''
    cart2d.Sendrecv([u[1:], 1, type_column], dest=neighbour[W], recvbuf=[u[(ex-sx+2):], 1, type_column], source=neighbour[E])

    ''' Send to neighbour E  and receive from neighbour W '''
    cart2d.Sendrecv([u[(ex-sx+1):], 1, type_column], dest=neighbour[E], recvbuf=[u, 1, type_column], source=neighbour[W])
...
def initialization(sx, ex, sy, ey):
    ...
    for iterx in range(sx, ex+1, 1):
        for itery in range(sy, ey+1, 1):
            u_exact[IDX(iterx, itery)] = func_u_exact(iterx, itery)
            f[IDX(iterx, itery)] = func_rhs_f(iterx, itery)
